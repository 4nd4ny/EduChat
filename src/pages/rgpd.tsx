...
//
// DEUX PHRASES ONT SUIVI LE CODE, et elles ne disaient plus la vérité :
//
// · LE POINT 3 ÉNUMÈRE CE QUE LE JOURNAL DE FACTURATION CONTIENT, et depuis le
//   prix par modèle il contient plus que « fournisseur et volume de tokens » :
//   usage_log porte aussi le MODÈLE appelé, les jetons d'entrée et de sortie
//   séparés, les deux prix appliqués (prix_entree_mtok, prix_sortie_mtok) et le
//   montant décompté — voir src/pages/api/completion.ts. Sous-déclarer ce qu'on
//   stocke est la seule erreur qu'une page de confidentialité n'a pas le droit
//   de faire, fût-ce dans le sens rassurant : le point 9 nommait déjà « le
//   fournisseur, le modèle et le montant » pour le porte-monnaie personnel, et
//   les deux points décrivent le même registre.
//
// · LE MOT DE PASSE DE SÉANCE n'ouvre plus « l'accès » en général. Le verrou
//   porte l'école qui l'a posé (src/server/access.ts, salleDepuisIp) : il se
//   saisit depuis le réseau de l'établissement et ne vaut que pour lui. La
//   phrase le dit maintenant, parce qu'un élève qui lit cette page doit y
//   trouver la même règle que son enseignant lit dans le guide.
    ...
    sections: [
      {
        ...
        body: [
          "No sign-up, no username, no password is ever asked of students. On the public site you write without entering anything: the free demonstration answers, when it is in service. Using your OWN API key there opens that demonstration's provider and nothing else: outside a school network, a key pasted by an anonymous visitor reaches no other provider. Through the school area (/school), access is opened by a teacher (session password, entered from the school's own network and valid for that network alone) or automatically from the school's IP address during agreed hours.",
      ...
      },
      {
        ...
        body: [
          ...
          "3. For internal-key billing: consumption per SCHOOL IP address (a school's address, not a person's), with the provider, the MODEL called, incoming and outgoing tokens counted separately, the two prices applied to them and the amount debited. Those two prices are frozen on the line at the moment of the call, which is what lets a school recompute its own invoice; none of these columns identifies a person. Personal-key usage is never logged with your IP.",
    ...
    sections: [
      {
        ...
        body: [
          "Nessuna registrazione, nessun identificativo, nessuna password viene richiesta agli studenti. Sul sito pubblico si scrive senza inserire nulla: risponde la dimostrazione gratuita, quando è in servizio. Usare la PROPRIA chiave API vi apre soltanto il fornitore di quella dimostrazione: fuori dalla rete di una scuola, una chiave incollata da un visitatore anonimo non raggiunge alcun altro fornitore. Nell'area istituti (/school), l'accesso è aperto da un insegnante (password di sessione, inserita dalla rete dell'istituto e valida per quella sola rete) o automaticamente dall'indirizzo IP della scuola negli orari concordati.",
      ...
      },
      {
        ...
        body: [
          ...
          "3. Per la fatturazione della chiave interna: il consumo per indirizzo IP dell'ISTITUTO (l'indirizzo di una scuola, non di una persona), con il fornitore, il MODELLO chiamato, i token d'entrata e d'uscita contati separatamente, i due prezzi che sono stati loro applicati e l'importo scalato. Questi due prezzi sono fissati sulla riga al momento della chiamata, ed è ciò che permette a una scuola di rifare da sé la propria fattura; nessuna di queste colonne designa una persona. Gli usi con chiave personale non vengono mai registrati con il Suo IP.",
    ...
    sections: [
      {
        ...
        body: [
          "Keine Registrierung, kein Benutzername, kein Passwort wird von Lernenden verlangt. Auf der öffentlichen Website schreibt man, ohne etwas einzugeben: Es antwortet die kostenlose Demonstration, sofern sie in Betrieb ist. Der EIGENE API-Schlüssel öffnet dort nur den Anbieter dieser Demonstration: ausserhalb eines Schulnetzes erreicht ein von einer anonymen Person eingefügter Schlüssel keinen anderen Anbieter. Im Schulbereich (/school) öffnet eine Lehrperson den Zugang (Sitzungspasswort, aus dem Netz der Schule eingegeben und nur für dieses Netz gültig) oder er öffnet sich automatisch von der IP-Adresse der Schule während der vereinbarten Zeiten.",
      ...
      },
      {
        ...
        body: [
          ...
          "3. Für die Abrechnung des internen Schlüssels: der Verbrauch pro SCHUL-IP-Adresse (die Adresse einer Schule, nicht einer Person), mit dem Anbieter, dem aufgerufenen MODELL, den getrennt gezählten eingehenden und ausgehenden Token, den beiden darauf angewandten Preisen und dem abgezogenen Betrag. Diese beiden Preise werden im Moment des Aufrufs auf der Zeile festgehalten — das ist es, was einer Schule erlaubt, ihre Rechnung selbst nachzurechnen; keine dieser Spalten bezeichnet eine Person. Nutzungen mit persönlichem Schlüssel werden nie mit Ihrer IP protokolliert.",
