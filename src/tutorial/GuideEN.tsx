...
const WALK: WalkStep[] = [
  ...
  },
  {
    ...
    text: "“Try” opens the chat with that tutor in charge (banner at the top). Without an account, the free demo answers: a small model, chosen by us and imposed — quite enough to judge a tutor. With a verified account, your personal API key (“Personal key” field) opens every provider: it stays in the page, is remembered only if you ask, and it unlocks attachments (images, PDF) and voice chat. Replies stream in live, as they are generated.",
    ...
    <>
      ...
      <Section id="eleves" title="Students and visitors — learning">
        <ul className="list-inside list-disc space-y-1">
          ...
          <li><b>Voice chat</b> (account + personal key, compatible providers): dictate your question into the microphone, and voice mode reads replies aloud — handy on a smartphone.</li>
      ...
      <Section id="acces" title="Which AI provider, for whom">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="align-top">
              ...
              <tr className="border-b border-white/5"><td className="py-1.5 pr-2"><b>From a school network</b></td><td className="pr-2">The compliant providers the school accepts, on its key.</td><td>The school's — unless your personal key is already attached to your account: then all of them.</td></tr>
            </tbody>
          </table>
        </div>
        ...
        <p><b>What an account changes.</b> Outside a school, a verified account opens the whole list, with your own key: you are identifiable, you pay for your calls, you answer for them. The three go together, and that is why a key pasted in by an anonymous visitor is no longer enough — there is nobody to ask anything of. Verification takes thirty seconds on <Link className="underline" href="/verifier">/verifier</Link>: a name, an address, a six-digit code, and <b>never a password</b>.</p>
        <p><b>On a school network.</b> There, the school answers for its students, and its rule applies to everyone, account or no account: only compliant providers are offered, and the school's key never pays for the others. One exception, and only one: an account whose personal key was <b>attached before coming</b> gets the whole list back — what an adult pays for themselves is no business of the school. A key simply typed into the field, on the spot, lifts nothing: otherwise a key found on a forum would reopen in class what the school has set aside.</p>
      ...
      <Section id="etablissement" title="Schools — the school at home">
        ...
        <p><b>Providers set aside.</b> Under the European AI Act, Grok, Gemini and the Chinese providers are offered to nobody on a school network, and the school's key never pays for them — no more for a teacher than for a student. OpenRouter carries a warning flag rather than an exclusion: it is only an intermediary towards compliant models, and it is also the engine of the free demo; a school, however, never pays for it. What an adult chooses for themselves, with their own account and key, does not go through the school's wallet and does not commit it — the section <a className="underline" href="#acces">“Which AI provider, for whom”</a> gives the whole rule.</p>
