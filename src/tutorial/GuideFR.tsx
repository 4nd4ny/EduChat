...
const WALK: WalkStep[] = [
  ...
  },
  {
    ...
    text: "« Essayer » ouvre le chat avec ce tuteur aux commandes (bandeau en haut). Sans compte, c'est la démonstration gratuite qui répond : un petit modèle, choisi par nous et imposé — largement de quoi juger un tuteur. Avec un compte vérifié, votre clé API personnelle (champ « Clé personnelle ») ouvre tous les fournisseurs hors du réseau d'une école — sur ce réseau, la règle de l'établissement s'applique à tous, sauf l'exception décrite plus bas. Elle reste dans la page, n'est mémorisée que si vous le demandez, et débloque les pièces jointes (images, PDF) ; la dictée vocale, elle, se paie aussi sur la clé de l'école quand celle-ci sert. Les réponses s'affichent en direct, au fil de la génération, chez les fournisseurs qui savent le faire ; celles de la démonstration gratuite, elles, arrivent d'un bloc.",
  ...
  },
  {
    ...
    text: "Une école s'inscrit elle-même : un nom, les adresses IP de son réseau, une adresse de facturation. Son porte-monnaie démarre à zéro, et une recharge se convient en nous écrivant : aucun moyen de paiement n'est encore branché sur le site, et rien n'est facturé aujourd'hui. Une fois provisionné, chaque appel y est décompté sans la moindre marge sur l'inférence, aux deux prix du modèle réellement appelé — celui de l'entrée et celui de la sortie, tels que l'éditeur les publie — la contribution de l'école, de 3,5 à 10 % qu'elle choisit, est retenue une seule fois, sur la recharge. L'accès se ferme seul quand le crédit est épuisé ; les élèves écrivent aux tuteurs sans compte ni clé. Le guide des établissements montre les parcours ; la section « Établissements » plus bas résume ce que l'école règle elle-même.",
    ...
    <>
      ...
      <Section id="eleves" title="Élèves et visiteurs — apprendre">
        <ul className="list-inside list-disc space-y-1">
          ...
          <li><b>Essayer</b> : le chat s'ouvre avec le tuteur aux commandes. <b>Sans compte</b>, c'est la <b>démonstration gratuite</b> qui répond — un petit modèle, imposé, de quoi juger un tuteur. <b>Avec un compte vérifié</b>, votre <b>clé API personnelle</b> (jamais stockée, sauf si vous demandez à la mémoriser) fait tourner la conversation, chez le fournisseur de votre choix. Les réponses arrivent alors <b>en direct</b>, au fil de la génération, chez les fournisseurs qui savent le faire (Gemini répond d'un bloc) ; celles de la démonstration gratuite arrivent elles aussi d'un bloc, une fois écrites. Les quatre situations sont détaillées <a className="underline" href="#acces">un peu plus bas</a>.</li>
          ...
          <li><b>Chat vocal</b> (fournisseurs à transcription : Mistral et ChatGPT) : dictez votre question au micro, et le mode vocal lit les réponses — pratique sur smartphone. La <b>dictée</b> se paie sur une clé qui peut payer : la vôtre, ou <b>celle de l'école tant qu'elle sert</b> — une classe n'a donc pas à en apporter une. La <b>lecture à voix haute</b>, elle, est la synthèse vocale du navigateur : aucune clé, aucun coût, rien d'envoyé. Les pièces jointes, à l'inverse, restent strictement réservées à la clé personnelle.</li>
      ...
      <Section id="acces" title="Quel fournisseur d'IA, pour qui">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="align-top">
              ...
              <tr className="border-b border-white/5"><td className="py-1.5 pr-2"><b>Du réseau d'une école</b></td><td className="pr-2">Les fournisseurs conformes que l'école accepte, sur sa clé.</td><td>Ceux de l'école — sauf si votre compte porte déjà son propre moyen de payer : alors tous.</td></tr>
            </tbody>
          </table>
        </div>
        ...
        <p><b>Ce qu'un compte change.</b> Hors d'une école, un compte vérifié ouvre toute la liste, avec votre propre clé : vous êtes identifiable, vous payez vos appels, vous en répondez. Les trois vont ensemble, et c'est pourquoi une clé collée par un visiteur anonyme ne suffit plus : on ne peut rien demander à personne. La vérification prend trente secondes sur <Link className="underline" href="/verifier">/verifier</Link> — un nom, une adresse, puis au choix le <b>code à six chiffres</b> reçu par courriel ou, plus court, le <b>lien du même message</b>, qui ouvre le compte d'un seul clic sans rien retaper. L'un et l'autre valent quinze minutes et une seule fois ; redemander un code périme le lien précédent. Et <b>jamais de mot de passe</b>.</p>
        <p><b>Sur le réseau d'une école.</b> Là, l'établissement répond de ses élèves, et sa règle s'applique à tout le monde, compte ou pas : seuls les fournisseurs conformes sont proposés, et la clé de l'école ne paie jamais les autres. Une exception : un compte qui porte <b>son propre moyen de payer, associé avant de venir</b>, retrouve toute la liste — ce qu'un adulte paie lui-même ne regarde pas l'école. Ce moyen se lit en base, et il y en a deux : une clé personnelle <b>mémorisée</b> sur le compte, ou un <b>crédit personnel</b> en cours — le porte-monnaie de <Link className="underline" href="/compte">/compte</Link>, dont le guichet de recharge n'est pas encore ouvert. Un particulier n'y a pas de curseur : il paie le taux plancher, <b>3,5 %</b>, retenu une fois à la recharge, et <b>jamais moins de 0.50</b> par recharge — sur la recharge minimale de 5, ce forfait vaut donc 10 %, et le relevé nomme lequel des deux a joué. Le crédit, lui, n'élargit que le menu : la clé d'EduChat ne sert jamais un fournisseur écarté ni à drapeau rouge, quel que soit le payeur, et pour atteindre ceux-là il faut sa propre clé. Une clé simplement tapée dans le champ, sur place, ne lève rien : sans cela, une clé trouvée sur un forum rouvrirait en classe ce que l'établissement a écarté.</p>
      ...
      <Section id="enseignants" title="Enseignants — la classe">
        ...
        <ul className="list-inside list-disc space-y-1">
          <li><b>Ouvrir la salle</b> : le mot de passe de salle déverrouille la clé de la plateforme pour la durée que vous choisissez. <b>Ce déverrouillage est borné au réseau local de votre établissement</b> : il vaut pour les élèves qui appellent depuis ce réseau, et pour eux seuls — ailleurs sur Internet, rien ne change. Chaque appel est reconnu par l'IP du réseau et décompté à l'école. Une durée vous est demandée pour que la salle se referme toute seule si on l'oublie, et vous pouvez la refermer avant l'heure. Deux écoles peuvent avoir leur salle ouverte en même temps sans rien se partager. Aucun élève n'a de compte ni de clé à saisir. Conséquence à connaître : <b>on n'ouvre pas la salle de chez soi</b> — le mot de passe n'ouvre que l'école du réseau d'où part la demande. Depuis la maison, vous pouvez en revanche préparer la séance et déployer un tuteur.</li>
          <li><b>Déployer un tuteur</b> : le tuteur choisi arrive pré-sélectionné chez chaque élève de la salle, avec ou sans recherche web. Vous pouvez aussi restreindre les fournisseurs de la séance ; tout décocher est un choix valable, et il ferme — la clé de l'école ne sert alors plus rien. Le filtre s'applique sur le serveur, à tout ce qui passe par la clé d'EduChat — celle de l'école, et aussi le crédit personnel d'un compte : payer soi-même ne lève pas ce que l'enseignant a coché pour son heure de cours. Seule une clé personnelle y échappe, parce qu'elle ne relève que de son titulaire. Celui qui l'apporte reste en revanche dans le périmètre de l'école : sur ce réseau, sa clé ne lui ouvre que les fournisseurs conformes, à moins qu'elle ne soit déjà associée à son compte.</li>
      ...
      <Section id="etablissement" title="Établissements — l'école chez elle">
        ...
        <p><b>Le porte-monnaie prépayé.</b> L'école provisionne une somme ; chaque appel passé sur la clé de la plateforme y est décompté <b>sans un centime de marge sur l'inférence</b> — c'est le point important, et c'est ce que le code garantit : le calcul se fait à taux zéro. Le prix appliqué est celui du <b>modèle réellement appelé</b>, entrée et sortie séparément — deux prix dont le rapport va de 1 à 5 chez tous les éditeurs, et qu'un prix unique ne pouvait pas représenter. Ils sont relevés au catalogue public d'OpenRouter pour les barreaux des trois fournisseurs conformes au RGPD, et chaque appel emporte sur sa ligne les deux prix qu'on lui a appliqués : une facture ne se recalcule jamais au tarif d'aujourd'hui, et un tarif qui change ce soir ne déplace pas d'un centime ce qui était dû ce matin. La facture nomme donc le modèle, sépare les jetons d'entrée et ceux de sortie, et affiche les deux prix : rouvrez le tarif public du modèle, multipliez, vous retrouvez notre chiffre. <b>Et si un modèle n'a pas de prix relevé</b> — un nom qui change chez l'éditeur, une sonde qui n'a pas encore tourné —, rien ne s'arrête et rien ne vaut zéro : l'appel est facturé au prix du barreau le plus cher connu de ce fournisseur, ce qui protège le budget dans le bon sens, et ce repli est écrit en toutes lettres sur la ligne de facture comme au registre du porte-monnaie. Ce que l'école peut vérifier elle-même, ce sont donc trois relevés : sa facture, ligne par ligne ; le registre de son porte-monnaie, mouvement par mouvement ; et le tableau des prix relevés (juste après). Une marge fondue dans le prix du jeton, à l'inverse, se vérifierait mal et se soupçonnerait bien. L'accès se ferme seul quand le crédit est épuisé, et le contrôle a lieu avant l'appel : une réponse déjà commencée se paie, le dépassement est donc borné par une réponse. L'écran montre le solde, la dépense des trente derniers jours, l'autonomie estimée en jours, la recharge suggérée, et l'historique daté des mouvements — recharge, consommation, ajustement. Le porte-monnaie, la facture et le tableau des prix ne s'affichent qu'aux <b>administrateurs de l'école</b>, et non à tout enseignant rattaché. <b>Aujourd'hui, rien n'est encore facturé</b> : aucun moyen de paiement n'est branché sur le site, et une recharge se convient en nous écrivant. En attendant, chacun peut apporter sa propre clé d'IA.</p>
        <p><b>Ce que coûte un million de jetons.</b> Sous la facture, un administrateur de l'école lit les barreaux de l'échelle de <b>son seul fournisseur actif</b> — celui que l'administration du site a inscrit pour elle —, en deux colonnes de prix, entrée et sortie, avec autant de lignes que l'échelle compte de barreaux. C'est un <b>relevé daté</b>, lu dans le catalogue public d'OpenRouter et converti dans la monnaie de facturation — ou laissé en dollars si le taux du jour n'a pas pu être lu, la monnaie voyageant alors avec le montant. Le lien « vérifier » rouvre cette page-là, celle du catalogue, et non le tarif de l'éditeur. Un prix qui n'a pas pu être relevé est dit comme tel, avec sa raison au survol, jamais affiché « 0.00 ». Ces prix-là sont bien ceux qui facturent, à la date où ils ont été relevés — mais ce tableau n'est pas pour autant une facture : il dit le prix du jour, tandis que chaque appel déjà passé a figé le sien. Ce qui a réellement été décompté se lit au registre du porte-monnaie et, ligne par ligne, sur la facture du mois.</p>
        ...
        <p><b>Les fournisseurs écartés.</b> Au titre du règlement européen sur l'IA, Grok, Gemini et les fournisseurs chinois ne sont proposés à personne qui relève de la règle de l'école sur son réseau — élèves et enseignants sans distinction. La seule liste qui les rouvre là est celle d'un compte portant déjà <b>son propre moyen de payer</b>, l'exception décrite <a className="underline" href="#acces">plus haut</a> ; et elle ne rouvre que le menu. Ce qui, lui, ne souffre aucune exception, c'est le payeur : <b>la clé de l'école ne les paie jamais</b>, pas davantage pour un enseignant que pour un élève, et la clé d'EduChat pas davantage sur un crédit personnel. OpenRouter, lui, n'est pas écarté d'un public scolaire — il n'est qu'un intermédiaire vers des modèles conformes, et c'est même le moteur de la démonstration gratuite —, mais il porte à ce titre d'intermédiaire un drapeau rouge, et cela suffit : sur le réseau d'une école, il n'est pas proposé davantage — sous la même réserve —, et la clé de l'école ne le paie jamais. Ce qu'un adulte choisit pour lui-même, avec son compte et sa clé, ne passe pas par le porte-monnaie de l'établissement et ne l'engage pas — la section <a className="underline" href="#acces">« Quel fournisseur d'IA, pour qui »</a> donne la règle entière.</p>
      ...
      <Section id="promptagogues" title="Promptagogues — créer un tuteur">
        ...
        <ol className="list-inside list-decimal space-y-2">
          <li><b>Identifiez-vous</b> (facultatif mais recommandé) sur <Link className="underline" href="/verifier">/verifier</Link> : nom public + email. Le message porte un <b>lien</b> qui ouvre le compte d'un seul clic, sans rien retaper, et le <b>code à six chiffres</b> pour qui lit son courrier sur un autre appareil — lien et code valent quinze minutes et une seule fois. <b>Aucun mot de passe, jamais.</b> Sans compte, la publication est anonyme — testable et soumissible via l'URL secrète, mais toute la modération (validation, dépublication, archivage) reviendra à l'administration.</li>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody>
              ...
              <tr className="border-b border-white/5"><td className="py-1 pr-2"><Link className="underline" href="/verifier">/verifier</Link></td><td className="pr-2">Auteurs, enseignants, admins</td><td>Identification par le lien ou le code reçus par email, sans mot de passe</td></tr>
              <tr className="border-b border-white/5"><td className="py-1 pr-2"><Link className="underline" href="/compte">/compte</Link></td><td className="pr-2">Comptes</td><td>Le COMPTE : consommation, clés, crédit personnel (recharge pas encore ouverte), conversations, tuteurs — tout exporter ou effacer</td></tr>
