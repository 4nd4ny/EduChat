...
const WALK: WalkStep[] = [
  ...
  },
  {
    ...
    text: "« Prova » apre la chat con quel tutor al comando (barra in alto). Senza account risponde la dimostrazione gratuita: un piccolo modello, scelto da noi e imposto — più che sufficiente per giudicare un tutor. Con un account verificato, la Sua chiave API personale (campo « Chiave personale ») apre tutti i fornitori fuori dalla rete di una scuola — su quella rete vale la regola dell'istituto per tutti, salvo l'eccezione descritta più in basso. Resta nella pagina, è memorizzata solo su Sua richiesta e sblocca gli allegati (immagini, PDF); la dettatura vocale, invece, si paga anche sulla chiave della scuola quando è essa a servire. Le risposte arrivano allora in diretta, durante la generazione, presso i fornitori che sanno farlo (Gemini risponde in blocco); quelle della dimostrazione gratuita, invece, arrivano tutte insieme, una volta scritte.",
  ...
  },
  {
    ...
    text: "Una scuola si iscrive da sé: un nome, gli indirizzi IP della sua rete, un indirizzo di fatturazione. Il suo portafoglio parte da zero, e una ricarica si concorda scrivendoci: nessun mezzo di pagamento è ancora collegato al sito, e oggi non viene fatturato nulla. Una volta alimentato, ogni chiamata vi viene scalata ai due prezzi del modello realmente chiamato — quello d'entrata e quello d'uscita, così come l'editore li pubblica —, senza alcun ricarico — il contributo della scuola, fra il 3,5 e il 10 % che sceglie lei stessa, è trattenuto una sola volta, sulla ricarica. L'accesso si chiude da solo quando il credito è esaurito; gli allievi scrivono ai tutor senza account né chiave. La guida degli istituti mostra i percorsi; la sezione « Istituti » più in basso riassume ciò che la scuola regola da sé.",
    ...
    <>
      ...
      <Section id="eleves" title="Studenti e visitatori — imparare">
        <ul className="list-inside list-disc space-y-1">
          ...
          <li><b>Chat vocale</b> (fornitori con trascrizione: Mistral e ChatGPT): detti la Sua domanda al microfono, e la modalità vocale legge le risposte — comoda sullo smartphone. La <b>dettatura</b> si paga su una chiave che possa pagare: la Sua, oppure <b>quella della scuola finché è essa a servire</b> — una classe non deve dunque portarne una. La <b>lettura ad alta voce</b>, invece, è la sintesi vocale del browser: nessuna chiave, nessun costo, nulla di inviato. Gli allegati, al contrario, restano strettamente riservati alla chiave personale.</li>
      ...
      <Section id="acces" title="Quale fornitore di IA, e per chi">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="align-top">
              ...
              <tr className="border-b border-white/5"><td className="py-1.5 pr-2"><b>Dalla rete di una scuola</b></td><td className="pr-2">I fornitori conformi che la scuola accetta, sulla sua chiave.</td><td>Quelli della scuola — salvo se il Suo account porta i propri mezzi (chiave personale collegata al conto, o credito personale): allora tutti.</td></tr>
            </tbody>
          </table>
        </div>
        ...
        <p><b>Che cosa cambia un account.</b> Fuori da una scuola, un account verificato apre l'intero elenco, con la Sua chiave: Lei è identificabile, paga le proprie chiamate, ne risponde. Le tre cose vanno insieme, ed è per questo che una chiave incollata da un visitatore anonimo non basta più: non c'è nessuno a cui chiedere conto. La verifica richiede trenta secondi su <Link className="underline" href="/verifier">/verifier</Link>: un nome, un indirizzo, poi il link o il codice a sei cifre che arrivano per email, e <b>mai una password</b>.</p>
        <p><b>Sulla rete di una scuola.</b> Lì l'istituto risponde dei propri allievi, e la sua regola vale per tutti, con o senza account: sono proposti solo i fornitori conformi, e la chiave della scuola non paga mai gli altri. Un'eccezione: un account che porta i propri mezzi — una chiave personale <b>collegata al conto prima di arrivare</b>, oppure un credito personale già versato (la ricarica, però, non è ancora aperta; un privato non ha cursore: paga l'aliquota minima, <b>3,5 %</b>, trattenuta una sola volta sulla ricarica, e <b>mai meno di 0.50</b> per ricarica — sulla ricarica minima di 5 quel minimo vale il 10 %, e l'estratto dice quale dei due ha giocato) — ritrova l'intero elenco, perché ciò che un adulto paga di tasca propria non riguarda la scuola. Una chiave semplicemente digitata nel campo, sul posto, non toglie nulla: altrimenti una chiave trovata su un forum riaprirebbe in classe ciò che l'istituto ha escluso. E l'elenco dice ciò che si può <i>scegliere</i>, non chi paga: per raggiungere un fornitore escluso occorre la propria chiave, perché la chiave di EduChat non ne serve mai uno, qualunque sia il pagatore.</p>
      ...
      <Section id="enseignants" title="Insegnanti — la classe">
        ...
        <ul className="list-inside list-disc space-y-1">
          <li><b>Aprire l'aula</b>: la password d'aula sblocca la chiave della piattaforma per la durata che sceglie Lei. <b>Questo sblocco è limitato alla rete locale del Suo istituto</b>: vale per gli allievi che chiamano da quella rete, e soltanto per loro — altrove su Internet non cambia nulla. Ogni chiamata è riconosciuta dall'IP della rete e scalata alla scuola. La durata Le viene chiesta perché l'aula si richiuda da sé se la si dimentica, e Lei può richiuderla prima del tempo. Due scuole possono avere la propria aula aperta nello stesso momento senza condividere nulla. Nessun allievo ha un account o una chiave da inserire. Una conseguenza da conoscere: <b>l'aula non si apre da casa</b> — la password apre soltanto la scuola della rete da cui parte la richiesta. Da casa può però preparare la seduta e distribuire un tutor.</li>
          <li><b>Distribuire un tutor</b>: il tutor scelto arriva preselezionato presso ogni allievo dell'aula, con o senza ricerca web. Può anche limitare i fornitori della seduta; togliere tutte le spunte è una scelta legittima, e chiude — la chiave della scuola non serve allora più nulla. Il filtro vale per la chiave della scuola e per un credito personale; un allievo che porta la propria chiave non ne dipende. Resta però nel perimetro della scuola: su quella rete la sua chiave gli apre soltanto i fornitori conformi, a meno che non sia già collegata al suo account.</li>
      ...
      <Section id="etablissement" title="Istituti — la scuola a casa sua">
        ...
        <p><b>Il portafoglio prepagato.</b> La scuola versa una somma; ogni chiamata effettuata sulla chiave della piattaforma vi viene scalata alla tariffa pubblica del modello impiegato — Anthropic, OpenAI, Mistral —, <b>senza un centesimo di ricarico</b>. È il punto che conta, e il calcolo si fa ad aliquota zero. Il prezzo applicato è quello del <b>modello realmente chiamato</b>, entrata e uscita separatamente — due prezzi il cui rapporto va da 1 a 5 presso tutti gli editori, e che un prezzo unico non poteva rappresentare. Vengono rilevati dal catalogo pubblico di OpenRouter per i gradini dei tre fornitori conformi al GDPR, e ogni chiamata porta sulla propria riga i due prezzi che le sono stati applicati: una fattura non viene mai ricalcolata alla tariffa di oggi, e una tariffa che cambia stasera non sposta di un centesimo ciò che era dovuto stamattina. La fattura nomina dunque il modello, separa i token d'entrata da quelli d'uscita e mostra entrambi i prezzi: riapra il tariffario pubblico del modello, moltiplichi, e ritroverà la nostra cifra. <b>E se un modello non ha un prezzo rilevato</b> — un nome cambiato presso l'editore, una sonda non ancora eseguita —, nulla si ferma e nulla vale zero: la chiamata viene fatturata al prezzo del gradino più caro conosciuto per quel fornitore, il che protegge il bilancio nel verso giusto, e questo ripiego è scritto per esteso sulla riga di fattura come sul registro del portafoglio. Ciò che la scuola può verificare da sé sono dunque tre estratti: la sua fattura, riga per riga; il registro del suo portafoglio, movimento per movimento; e la tabella dei prezzi rilevati (subito qui sotto). Un margine sciolto nel prezzo del token, al contrario, si verificherebbe male e si sospetterebbe bene. L'accesso si chiude da solo quando il credito è esaurito, e il controllo avviene prima della chiamata: una risposta già iniziata si paga, quindi lo sconfinamento è limitato a una risposta. La schermata — riservata agli amministratori della scuola, come la fattura — mostra il saldo, la spesa degli ultimi trenta giorni, l'autonomia stimata in giorni, la ricarica suggerita e la cronologia datata dei movimenti — ricarica, consumo, rettifica. <b>Oggi non viene ancora fatturato nulla</b>: nessun mezzo di pagamento è collegato al sito, e una ricarica si concorda scrivendoci. Nel frattempo ognuno può portare la propria chiave IA.</p>
        {/* AJOUT : le tableau des prix relevés manquait entièrement à ce guide,
            alors que les trois autres le décrivent. Repris du guide de
            référence, GuideFR — fournisseur actif seul, deux colonnes, autant
            de lignes que de barreaux, relevé daté lu chez OpenRouter, prix
            manquant dit et non « 0.00 », et ce n'est pas une facture. */}
        <p><b>Quanto costa un milione di token.</b> Sotto la fattura, un amministratore della scuola legge i pioli della scala del <b>suo solo fornitore attivo</b> — quello che l'amministrazione del sito ha registrato per lei —, in due colonne di prezzo, entrata e uscita, con tante righe quanti sono i pioli della scala. È un <b>rilevamento datato</b>, letto nel catalogo pubblico di OpenRouter e convertito nella moneta di fatturazione — o lasciato in dollari se il tasso del giorno non si è potuto leggere, e allora la moneta viaggia con l'importo. Il link « verificare » riapre quella pagina lì, quella del catalogo, e non il tariffario dell'editore. Un prezzo che non si è potuto rilevare viene detto tale, con la sua ragione al passaggio del mouse, e non mostrato « 0.00 ». Questi prezzi sono proprio quelli che fatturano, alla data in cui sono stati rilevati — ma questa tabella non è per questo una fattura: dice il prezzo di oggi, mentre ogni chiamata già passata ha fissato il proprio. Ciò che è stato realmente scalato si legge nel registro del portafoglio e, riga per riga, sulla fattura del mese.</p>
        ...
        <p><b>I fornitori esclusi.</b> Ai sensi del regolamento europeo sull'IA, Grok, Gemini e i fornitori cinesi non sono proposti a nessuno sulla rete di un istituto, e la chiave della scuola non li paga mai — non più per un insegnante che per un allievo. OpenRouter non è escluso in quanto tale da un pubblico scolastico — è solo un intermediario verso modelli conformi, ed è perfino il motore della dimostrazione gratuita pubblica —, ma proprio in quanto intermediario porta un contrassegno di avvertimento, e tanto basta: nemmeno lui figura nell'elenco proposto sulla rete di un istituto, e la chiave della scuola non lo paga mai. Per servirsene occorre la <b>propria chiave</b>: un credito personale non basta, perché la chiave di EduChat non serve mai un contrassegnato, chiunque sia a pagare. Ciò che un adulto sceglie per sé, con il proprio account e la propria chiave, non passa dal portafoglio dell'istituto e non lo impegna — la sezione <a className="underline" href="#acces">« Quale fornitore di IA, e per chi »</a> espone la regola per intero.</p>
      ...
      <Section id="promptagogues" title="Promptagoghi — creare un tutor">
        ...
        <ol className="list-inside list-decimal space-y-2">
          <li><b>Si identifichi</b> (facoltativo ma consigliato) su <Link className="underline" href="/verifier">/verifier</Link>: nome pubblico + email. Il messaggio porta un <b>link</b> che apre il conto con un solo clic, senza ridigitare nulla, e il <b>codice a sei cifre</b> per chi legge la posta su un altro apparecchio — link e codice scadono entrambi dopo quindici minuti e servono una volta sola. <b>Nessuna password, mai.</b> Senza account, la pubblicazione è anonima — testabile e inviabile tramite l'URL segreto, ma tutta la moderazione (validazione, ritiro, archiviazione) spetterà all'amministrazione.</li>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody>
              ...
              <tr className="border-b border-white/5"><td className="py-1 pr-2"><Link className="underline" href="/verifier">/verifier</Link></td><td className="pr-2">Autori, insegnanti, admin</td><td>Identificazione con il link o il codice ricevuti per email, senza password</td></tr>
              <tr className="border-b border-white/5"><td className="py-1 pr-2"><Link className="underline" href="/compte">/compte</Link></td><td className="pr-2">Account</td><td>L'ACCOUNT: consumo, chiavi, credito personale (ricarica non ancora aperta), conversazioni, tutor — esportare o cancellare tutto</td></tr>
