...
const WALK: WalkStep[] = [
  ...
  },
  {
    ...
    text: "Una scuola si iscrive da sé: un nome, gli indirizzi IP della sua rete, un indirizzo di fatturazione. Il suo portafoglio parte da zero, e una ricarica si concorda scrivendoci: nessun mezzo di pagamento è ancora collegato al sito, e oggi non viene fatturato nulla. Una volta alimentato, ogni chiamata vi viene scalata alla tariffa registrata per quel fornitore, senza alcun ricarico — il contributo della scuola, fra il 3,5 e il 10 % che sceglie lei stessa, è trattenuto una sola volta, sulla ricarica. L'accesso si chiude da solo quando il credito è esaurito; gli allievi scrivono ai tutor senza account né chiave. La guida degli istituti mostra i percorsi; la sezione « Istituti » più in basso riassume ciò che la scuola regola da sé.",
  ...
  return (
    <>
      ...
      <Section id="enseignants" title="Insegnanti — la classe">
        ...
        <ul className="list-inside list-disc space-y-1">
          <li><b>Aprire l'aula</b>: la password d'aula sblocca la chiave della piattaforma per la durata che sceglie Lei. <b>Sappia che questo sblocco non è limitato alla Sua rete</b>: finché dura, quella chiave risponde anche a qualsiasi account verificato, da qualunque luogo chiami — per questo Le viene chiesta una durata, e per questo può richiudere prima del tempo. In aula la chiamata è riconosciuta dall'IP della rete e scalata alla scuola; fuori, non lo è. Nessun allievo ha un account o una chiave da inserire.</li>
      ...
      <Section id="etablissement" title="Istituti — la scuola a casa sua">
        ...
        <p><b>Il portafoglio prepagato.</b> La scuola versa una somma; ogni chiamata effettuata sulla chiave della piattaforma vi viene scalata alla tariffa che l'amministrazione del sito registra per ciascun fornitore — Anthropic, OpenAI, Mistral —, <b>senza un centesimo di ricarico</b>. È il punto che conta, e il calcolo si fa ad aliquota zero. Il prezzo applicato, però, non viene ripreso da solo presso l'editore: è un prezzo unico al milione di token che l'amministrazione del sito iscrive a mano, lo stesso per i token d'entrata e per quelli d'uscita, là dove il catalogo di un editore distingue le due colonne. Riaprire il tariffario di Anthropic, di OpenAI o di Mistral non restituisce dunque la nostra cifra al centesimo; e finché nessun prezzo è stato iscritto per un fornitore, non viene scalato nulla. Ciò che la scuola può verificare da sé sono due estratti: il registro del suo portafoglio, movimento per movimento, e la tabella dei prezzi rilevati (subito qui sotto). Un margine sciolto nel prezzo del token, al contrario, si verificherebbe male e si sospetterebbe bene. L'accesso si chiude da solo quando il credito è esaurito, e il controllo avviene prima della chiamata: una risposta già iniziata si paga, quindi lo sconfinamento è limitato a una risposta. La schermata — riservata agli amministratori della scuola, come la fattura — mostra il saldo, la spesa degli ultimi trenta giorni, l'autonomia stimata in giorni, la ricarica suggerita e la cronologia datata dei movimenti — ricarica, consumo, rettifica. <b>Oggi non viene ancora fatturato nulla</b>: nessun mezzo di pagamento è collegato al sito, e una ricarica si concorda scrivendoci. Nel frattempo ognuno può portare la propria chiave IA.</p>
