...
const WALK: WalkStep[] = [
  ...
  {
    // Le résumé suit le paragraphe « Das Guthaben im Voraus. » plus bas : il
    // promet de nouveau le prix de l'éditeur parce que c'est de nouveau vrai —
    // les deux prix du MODÈLE appelé, relevés au catalogue public et figés sur
    // chaque ligne de facture. Formulation reprise du guide de référence,
    // GuideFR.
    title: "Vertreten Sie eine Schule?",
    text: "Eine Schule meldet sich selbst an: ein Name, die IP-Adressen ihres Netzes, eine Rechnungsadresse. Ihr Guthaben startet bei null, und eine Aufladung wird vereinbart, indem sie uns schreibt: Es ist noch kein Zahlungsmittel an die Website angebunden, und heute wird nichts verrechnet. Ist es aufgefüllt, wird jeder Aufruf ohne die geringste Marge auf die Inferenz davon abgezogen — zu den beiden Preisen des tatsächlich aufgerufenen Modells, Eingang und Ausgang getrennt, wie der Anbieter sie veröffentlicht — der Beitrag der Schule, 3,5 bis 10 % nach ihrer eigenen Wahl, wird einmalig bei der Aufladung einbehalten. Der Zugang schliesst sich von selbst, sobald das Guthaben erschöpft ist; die Schüler schreiben ohne Konto und ohne Schlüssel an die Tutoren. Der Schul-Leitfaden zeigt die Abläufe; der Abschnitt „Schulen“ weiter unten fasst zusammen, was eine Schule selbst regelt.",
  ...
  return (
    <>
      ...
      <Section id="enseignants" title="Lehrpersonen — die Klasse">
        ...
        <ul className="list-inside list-disc space-y-1">
          <li><b>Den Raum öffnen</b>: Das Raumpasswort entsperrt den Schlüssel der Plattform für die von Ihnen gewählte Dauer. <b>Diese Entsperrung ist auf das lokale Netz Ihrer Schule begrenzt</b>: Sie gilt für die Schülerinnen und Schüler, die aus diesem Netz aufrufen, und nur für sie — anderswo im Internet ändert sich nichts. Jeder Aufruf wird an der IP des Netzes erkannt und der Schule abgezogen. Die Dauer wird verlangt, damit sich der Raum von selbst schliesst, wenn man ihn vergisst, und Sie können ihn vorzeitig schliessen. Zwei Schulen können ihren Raum gleichzeitig offen haben, ohne etwas zu teilen. Kein Schüler braucht ein Konto oder einen Schlüssel. Eine Folge, die man kennen sollte: <b>von zu Hause aus lässt sich der Raum nicht öffnen</b> — das Passwort öffnet nur die Schule jenes Netzes, aus dem die Anfrage kommt. Von zu Hause können Sie hingegen die Lektion vorbereiten und einen Tutor bereitstellen.</li>
      ...
      <Section id="etablissement" title="Schulen — die Schule bei sich">
        ...
        {/* La promesse « ouvrez leur tarif, refaites le calcul, vous retrouvez
            notre chiffre » est de nouveau tenue — et c'est le CODE qui a
            changé, pas la phrase. Le décompte emploie les deux prix du MODÈLE
            réellement appelé (tarifs_modeles, écrit par la sonde depuis le
            catalogue public), et chaque appel fige ces deux prix sur sa ligne
            de journal : une facture ne se recalcule plus au tarif du jour.
            Le repli — barreau le plus cher connu du fournisseur — est nommé
            ici parce qu'il est nommé sur la facture. */}
        <p><b>Das Guthaben im Voraus.</b> Die Schule zahlt einen Betrag ein; jeder Aufruf über den Schlüssel der Plattform wird <b>zum Selbstkostenpreis</b> davon abgezogen — Anthropic, OpenAI, Mistral —, ohne einen Rappen Aufschlag auf die Inferenz. Der angewandte Preis ist jener des <b>tatsächlich aufgerufenen Modells</b>, Eingang und Ausgang getrennt — zwei Preise, deren Verhältnis bei allen Anbietern von 1 bis 5 reicht und die ein einziger Preis nicht abbilden konnte. Sie werden für die Sprossen der drei DSGVO-konformen Anbieter aus dem öffentlichen Katalog von OpenRouter abgelesen, und jeder Aufruf trägt auf seiner Zeile die beiden Preise, die ihm verrechnet wurden: Eine Rechnung wird nie zum heutigen Tarif neu berechnet, und ein Tarif, der heute Abend ändert, verschiebt keinen Rappen dessen, was heute Morgen geschuldet war. Die Rechnung nennt also das Modell, trennt eingehende von ausgehenden Jetons und zeigt beide Preise: Öffnen Sie die öffentliche Preisseite des Modells, rechnen Sie nach, und Sie erhalten unsere Zahl. <b>Und wenn für ein Modell kein Preis abgelesen ist</b> — ein beim Anbieter geänderter Name, eine noch nicht gelaufene Sonde —, hält nichts an und nichts gilt als null: Der Aufruf wird zum Preis der teuersten bekannten Sprosse dieses Anbieters verrechnet, was das Budget in die richtige Richtung schützt, und dieser Rückgriff steht ausgeschrieben auf der Rechnungszeile wie im Register des Guthabens. Eine im Tokenpreis aufgelöste Marge liesse sich schlecht prüfen und gut verdächtigen; deshalb steht dort keine. Geprüft wird vor dem Aufruf: Eine bereits begonnene Antwort wird bezahlt, die Überschreitung ist also durch eine Antwort begrenzt. Der Bildschirm zeigt den Saldo, die Ausgaben der letzten dreissig Tage, die geschätzte Reichweite in Tagen, die vorgeschlagene Aufladung und den datierten Verlauf der Bewegungen — Aufladung, Verbrauch, Korrektur. <b>Heute wird noch nichts verrechnet</b>: Es ist kein Zahlungsmittel an die Website angebunden, und eine Aufladung wird vereinbart, indem Sie uns schreiben. In der Zwischenzeit kann jede und jeder den eigenen KI-Schlüssel mitbringen.</p>
        ...
        <p><b>Was die Schule einstellt, und wer es einstellt.</b> Zeiten des freien Zugangs, ein tägliches Kontingent pro Schüler, eine monatliche Token-Obergrenze und der Verbrauch des Monats nach Anbieter. Jede zugeordnete Lehrperson liest diesen Bildschirm; nur eine Administratorin der Schule ändert etwas daran, und die Seite sagt es, statt es entdecken zu lassen. Das Guthaben, die Preisleiter, die Rechnung, die Konten und die Tutoren der Schule erscheinen überhaupt nur ihr. Die Preisleiter zeigt die Modelle ihres aktiven Anbieters, Eingang und Ausgang getrennt, in Rechnungswährung, mit dem Datum der Ablesung und einem Link auf den öffentlichen Katalog von OpenRouter, wo diese Preise abgelesen wurden: eine datierte Ablesung. Es sind genau die Preise, die verrechnen — aber die Tabelle ist trotzdem keine Rechnung: Sie nennt den heutigen Preis, während jeder bereits getätigte Aufruf seinen eigenen festgehalten hat. IP-Adressen und Abrechnungsstatus bleiben in der Hand der Website.</p>
